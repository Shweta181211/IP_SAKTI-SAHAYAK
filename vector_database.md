...
This module converts the legal chunks in `output/02_chunks/all_chunks.json` into multilingual semantic embeddings and stores them in persistent local ChromaDB collection `ip_sakti_corpus`.
...
Import `search` from `test_retrieval.py`, load the model and collection once at application startup, then call:
